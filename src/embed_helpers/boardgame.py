...
class BoardGameObj:
    ...
    def createFromBGG(bggDict: dict, extraData: dict = None):
        ...
        return BoardGameObj(
            ...
            rank=int(rank if rank != "Not Ranked" else "-1"),
            averageRating=float(safeGet(bggDict, "statistics/ratings/average/@value", "-1.0")),
            bggRating=float(safeGet(bggDict, "statistics/ratings/bayesaverage/@value", "-1.0")),
    ...
    def getEmbed(self, flags: [str]) -> Embed:
        ...
        if "compact" not in flags:
            ...
            if self.averageRating == -1.0:
                ...
                embed.add_field(name="Average Rating", value=f"{round(self.averageRating, 2)}", inline=True)

            if self.bggRating == -1.0:
                embed.add_field(name="BGG Rating", value=f"Unknown", inline=True)
            else:
                embed.add_field(name="BGG Rating", value=f"{round(self.bggRating, 2)}", inline=True)
